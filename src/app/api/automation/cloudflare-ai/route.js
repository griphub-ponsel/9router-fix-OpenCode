// Cloudflare Workers AI auto-setup: take Global API Key + email, fetch account,
// resolve Workers AI Read/Edit + Account Analytics:Read permission groups, create
// a scoped API token, and upsert it into providerConnections.
//
// Body: { globalApiKey, email, tokenName? }
// Response: { ok, accountId, accountName, tokenId, connectionId, message }
...
export async function POST(request) {
  try {
    ...
    const { globalApiKey, email, tokenName, password } = body || {};

    // Optional auth — same password-header pattern used by /api/settings/database
    ...
    if (!cleanGAK || cleanGAK.length < 32) {
      return NextResponse.json(
        {
          error:
            "Invalid Global API Key format. Must be your Cloudflare Global API Key (37 hex chars), not a scoped token.",
        },
        { status: 400 }
      );
    }
    ...
    if (!accounts || accounts.length === 0) {
      return NextResponse.json(
        { error: "No Cloudflare accounts found for this credential" },
        { status: 400 }
      );
    }
    ...
    const editGroup =
      ...
      permGroups.find(
        (g) =>
          g.name?.toLowerCase().includes("workers ai") &&
          (g.name?.toLowerCase().includes("write") ||
            g.name?.toLowerCase().includes("edit")) &&
          !g.name?.toLowerCase().includes("metadata")
      );

    // Account Analytics:Read for GraphQL quota tracking (neuronsUsed per day)
    const analyticsReadGroup = permGroups.find(
      (g) =>
        g.name === "Account Analytics Read" ||
        g.name === "Account Analytics:Read" ||
        (g.name?.toLowerCase().includes("account analytics") &&
          g.name?.toLowerCase().includes("read"))
    );
    ...
    if (!readGroup) {
      return NextResponse.json(
        {
          error: `Workers AI permission groups not found. Available: ${permGroups
            .map((g) => g.name)
            .join(", ")}`,
        },
        { status: 400 }
    ...
    const finalReadGroup = readGroup;
    const finalEditGroup = editGroup || readGroup;
    ...
    // 3. Create scoped token
    const tokenPayload = {
      name: tokenName || `9router Workers AI - ${cleanEmail}`,
      policies: [
        {
          effect: "allow",
    ...
    return NextResponse.json(
      { error: err?.message || "Failed to setup Cloudflare Workers AI" },
      { status: 500 }
    );
  }
}
